...
public class Client {
	...
	public void clientStart() {
		// 1) 서버의 ip주소와 서버가 정한 포트번호를 매개변수로 하여 클라이언트용 소켓객체 생성
		int port = 30027;
		String str = null;
		String serverIp = null;
		Socket socket = null;
		serverIp = "192.168.130.21";

			// serverIp = InetAddress.getLocalHost().getHostAddress();

			try {
				// 서버에 연결을 보내기 (요청하고자하는 서버의 ip주소와 포트번호를 동시에 제시해주면서 객체 생성)
				...
				if (socket != null) { // 연결 성공
					...
					do {
						System.out.println("서버에 보낼 내용을 입력해주세요 :: ");
