...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 *  								   연결 요청이 오면 Socket를 반납 
		 *  
		 *  Socket
		 *  - 클라이언트에서 서버에 요청을 하거나, 서버에서 클라이언트의 요청을 받았을 떄 사용하는클래스
		 *  - 사용법 : Socket socket = new Socket(String ip주소, int portNumber)
		 *  
		 *  - 메소드
		 *  > OutputStream out = client.getOutputStream(); 이 메소드를 통해 데이터를 내보낼 수 있는 OutStream객체 생성가능
		 *  > InputStream in = client.getInputStream(); 이 메소드를 통해 데이터를 읽을 수 있는 InputStream객체 생성가능
		 * 
		 * 서버의 연결 순서
		 * 1) 서버의 소켓 객체 생성
		 ...
		 * 5) 클라이언트의 정보를 통해서 InputStream객체 생성
		 * 6) 스트림을 통해 입, 출력
		 * 7) 통신종료
		 */
