...
public class Run {
	...
	//URL 객체 생성 후 연결요청, 파일로 저장
	public void test3() {
		String address = "https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=%EC%95%88%EB%85%95";
	...
	/* URL : 인터넷에 존재하는 서버들으 ㅣ자원에 접근할 수 있는 주소를 다루는 클래스
	 * 		 실제 URL에 대한 객체 생성. 입력스트림과 연동하여 URL에 존재하는 데이터를 읽어올 수 있음.
	 * 
	 * protocol + hostname + port + 자원path 
	 * 
	 * protocol : 통신규약 http, https 
	 * 
	 * port : 서비스번호. 컴퓨터내에 특정 프로그램을 가리키는 논리적인 번호. 기본포트번호는 생략가능
	 * 				  http : 80
	 * 				  https : 443
	 * 				  ftp : 20
	 * 
	...
	public void test2() {
		try {
			URL url = new URL("https://search.naver.com/search.naver?where=nexearch&sm=top_hty&fbm=1&ie=utf8&query=%EC%95%88%EB%85%95");
			System.out.println(url.getProtocol());
			System.out.println(url.getHost());
			System.out.println(url.getPort() != -1? url.getPort() : url.getDefaultPort());
			System.out.println(url.getPath());
			System.out.println(url.getQuery());  // 퀴리스트링 :: 사용자가 입력한 입력값 
