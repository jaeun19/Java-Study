...
public class C_DimensionalArray {
	...
	public void method1() {
		...
		/*
		 * 이차원 배열 할당(크기 지정) 배열명 = new 자료형 [행크기][열크기];
		 */
		...
		System.out.println(arr); // 2차원 배열의 구조값
		System.out.println(arr[0]); // 1차원 배열의 구조값
		...
		/*
		 * 출력(arr[0][0]); 출력(arr[0][1]); ... 출력(arr[0][4]); ============= 출력(arr[1][0]);
		 * 출력(arr[1][1]); ... 출력(arr[1][4]); =============== 출력(arr[2][0]);
		 * 출력(arr[2][1]); ... 출력(arr[2][4]);
		 */
	...
	public void method4() {
		/*
		 * 가변 배열 행의 크기는 정해져있으나 각각의 행 별 열의 갯수가 정해지지 않은 상태 이차원 배열 == 일차원 배열 여러개 묶은 형태 즉,
		 * 묶여있는 일차원 배열의 길이가 같을 필요는 없다 행 크기는 생략이 뷸가능하지만, 열크기는 사실 생략이 가능하다. 각 행의 열의 길이가 변경
		 * 가능하다.
		 * 
		 */
...
//		System.out.println(arr[0].length);//nullpointer
	...
	public void method5() {
		...
		arr[0] = new char[3];
		arr[1] = new char[2];
		arr[2] = new char[4]; // j인덱스로 접근하려면 열 만들어줘야함
	...
	public void method7() {
		/*
		 * int 형 2차원 배열을 만들어 0번 행에는 국어점수 3개를 입력받고 1번 행에는 영어점수를 3개 입력 받은후
		 * 
		 * 각각 반복문을 통해 출력
		 * 
		 * 입력예시 : xx점수를 입력하세요 : 출력예시 : xx점수 : xx xx xx
		 */
