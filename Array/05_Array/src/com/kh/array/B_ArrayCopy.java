...
public class B_ArrayCopy {
	...
	public void method1() {
		...
		/*
		 * 복사본 배열만을 가지고 수정을 했는데, 원본배열도 같이 수정이 되었다. 왜냐면 얕은복사이기 때문이고, 배열의 주소값은 하나로 공유해서
		 * 사용하고 있기때문. 원본배열과 ,복사본 배열 모두 하나의 주소값을 가르키고 있다.
		 */
	...
	public void method2() {
		// 1. for문 활용하는 방법(수작업)
		//새로운 배열을 생성하고, 인덱스별 내부값을 일일이 대입하는 방법
		int[] origin = { 1, 2, 3, 4, 5 };
	...
	// 이미 만들어져있는 퀄리티 꽤 좋은 메서드 많이 있음.
	
	public void method3() {
		// 2. 새로운 배열을 생성한 후 System 클래스에서 arraycopy메서드를 이용한 복사
		
		int [] origin = {1,2,3,4,5};
		int [] copy = new int[10];

		// System.arraycopy(원본 배열명, 원본 배열에서 복사를 시작할 인덱스, 복사본 배열명, 복사본 배열의 복사가 시작될 인덱스,
		// 복사할 개수);
	...
	public void method4() {
		...
		int [] copy = Arrays.copyOf(origin, 7); //복사할 갯수 == copy배열의 크기
		
		System.out.println(Arrays.toString(copy));
		
		/*
		 * 2. System.arraycopy 메소드 : 몇번 인덱스부터 몇개를 어느 위치의 인덱스에 복사할건지 다 직접 저장 가능
		 * 
		 * 3. Arrays.copyOf 메소드 : 무조건 원본배열 0번 인덱스부터 내가 지정한 갯수만큼 복사가 진행됨
	...
	public void method5() {
		...
		// 복사본 배열 = 원본 배열.clone();
		int [] copy = origin. clone(); //인덱스 x, 복사할 갯수x, 원본배열과 똑같이 생성
		
		// clone()함수는 Arrays클래스에서만 깊은복사를 지원함
