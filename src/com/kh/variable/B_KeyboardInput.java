...
public class B_KeyboardInput {
	...
	public void inputTest1() {
		/*
		 * 자바에서 이미 제공하고 있는 Scanner클래스를 이용해야한다.
		...
		//String name = sc. next(); //입력값 전달받아서 엔터치면 name에 저장 
		//next() : 사용자가 입력한 값 중 "공백"이 있을 경우 공백 이전까지만 출력해줍니다.
		String name = sc.nextLine(); // 사용자가 엔터를 입력하기 전까지 대기상태
		//nextLine() : 사용자가 입력한 값 중 개행(엔터, \n)이 있을 경우 공백에 무관하게 개행이전까지만 출력해준다. 
		...
		sc.nextLine(); //입력버퍼에 남아있는 엔터를 비워주기 위해서 실행 (이해못함)
	 ...
	 * 
	 * 2) '문자'값을 입력받아야될 경우
	 * sc.nextLine()메소드로 우선 문자열을 입력받은 후 그뒤에 .charAt(인덱스)메소드를 통해 문자 하나만추출
	 * 
	 */
}
