...
public class Student {
	...
	public void setName(String name) {
		// 매개변수 : 해당 메소드 호출시 전달되는 값을 받아주기 우한 변수 선언문(이 메소드 안에서만 사용가능)
		
		// name = name; ->필드명과 매개변수 명이 동일한 시점에서 {}영역 내에 해당 변수를 제시시 해당영역 안에서 
		// 생성된 변수가 우선순위가 더 높다. 
		
		//this? 현재 생성된 객체의 주소값이 담겨있음.
		this.name = name; //필드명 찾아가기 위한 거 
	...
	// 데이터를 반환해주는 기능의 메소드 : getter메소드 ->필수
	
	//name 필드에 담긴 값을 돌려주는 용도의 메소드
	
	public String getName() {//반환시키고자 하는 자료형 넣으면된다.(String)
		return name; // return 결과값; 결과값을 돌려주겠다. // 무조건 위 필드에 있는 name이기 때문에 this없어도돼요
	...
	public String toString() {
		return name + "님의 학번은"+stdNo+"이고, 키는 "+height+"입니다"; // 결과값은 반드시 딱 한개여야함.
		
	}

	
	//stdNo 학번 필드에 담긴 값을 돌려주는 용도의 메소드
