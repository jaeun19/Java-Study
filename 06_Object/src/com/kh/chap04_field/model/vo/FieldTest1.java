 ...
 * 
 * -지역 변수 : 클래스 영역 내의 어떤 특정한 구역({})에 선언한 변수(선언한 지역에서만 사용 가능)
 ...
 *    소멸시점 : 객체 소멸시 같이 소멸
 *   
 *  - 클래스 변수 == static변수 : static이라는 예약어가 붙어있냐 아니냐에 다라 멤벼변수냐 클래스변수냐로 구분
 *    생성시점 : 프로그램 실행과 동시에(해당 객체 생성이 안돼도) 메모리 영역(static영역)에 할당
 *    소멸시점 : 프로그램이 종료될때 소멸
 *   
 * 2. 지역변수
 * 	  생성시점 : 특정한 구역({}) 실행(호출)시 메모리 영역에 하랑(ex)메소드 실행시)
...
public class FieldTest1 { // 클래스 시작부분, 
	...
	private int global; // 멤버변수 == 인스턴스 == 필드
	...
	public void test(int num) { //test메소드 시작
		// 매개변수(지역변수)는 자동으로 초기화되어있음.
		
		// 지역변수(메소드 영역내에 생성) -> 접근제한자를 붙일수가 없다
		int l = 0; // 지역변수 초기화시켜야함 . 값이 비어있기때문에 명시적으로 0 넣어주기
		
		if(true) {
			int a = 5; // 지역변수
		}
		//해당 클래스 객체 생성시 jvm이 값을 자동으로 초기화 시켜주기 때문에 에러 안남
		System.out.println(global);
		
		System.out.println(l);
		//매개변수 초기화하지 않아도 에러나지 않는 이유 : 해당 메소드를 호출할 때 "반드시"값을 넘겨줘야 실행이 되기 때문
