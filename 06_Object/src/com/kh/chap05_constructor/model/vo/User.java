...
public class User {
	 ...
	 * 생성자를 작성하는 목적
	 * 1. 객체를 생성해주기 위한 목적
	 * 2. 객체 생성 뿐만아니라 매개변수로 전달된 값을 곧바로 필드에 초기화할 목적
	 * 
	 * 생성자 작성시 주의사항
	 * 1. 반드시 클래스명과 동일해야함(대/소문자 구분)
	 * 2. 반환형이 존재하지 않는다(메소드와 비슷하게 생겨서 헷갈릴 수 있음)
	 * 3. 여러개 생성이 가능하지만 매개변수가 중복되면 안된다.
	 * 4. 매개변수 생성자를 명시적으로 작성하게 되면 기본 생성자를 jvm이 자동으로 만들어주지 않음.
	 * 
	 * 즉, 어찌됐든 기본 생성자는 항상 작성하는 습관을 들이자.
	 * 
	 */
	
	public User() { //매개변수 없는 생성자 == 기본생성자
		// 단지 객체 생성만을 목적으로 할 때 사용
		// 기본 생성자 작성하지 않아도 호출하는데 문제 없음
		// jvm이 자동으로 만들어줬기 때문에 항상 객체생성이 가능했다.
		// 단, 매개변수가 있는 생성자가 작성되어있을 때에는 기본생성자도 반드시 기술해야한다.중요!!
