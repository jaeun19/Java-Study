package com.kh.chap01_poly.part01_basic.model.run;
...
public class Run {
	...
	public static void main(String[] args) {
		...
		System.out.println("2. 자식타입 참조변수로 자식 객체를 다루는 경우");
		Child1 c1 = new Child1();
		c1.printChild1(); // 
		c1.printParent(); // 형변환 개념 //((Parent)c1).printParent();
		// c1참조변수로 Parent, Child1 주소값에 둘다 접근 가능.
		// (Parent 내부로 접근시 자동으로 형변환 된채로 진행된다)
		// 자식에서 부모로 자동형변환이됨. (작은것에서 큰것으로 형변환된것과 같은이치)
		
		//3. 부모타입 참조변수로 자식객체를 다루는 경우(다형성이 적용된 개념)
		System.out.println("3. 부모타입 참조변수로 자식객체를 다루는 경우(다형성이 적용된 개념)");
		Parent p2 =/*(Parent)*/ new Child1();
		// 대입연산자기준 양쪽의 자료형이 다름에도 불구하고 오류가 발생하지 않음/
		// 자식객체인 Child1이 부모클래스인 Parent객체로 '자동형변환'이 되고 있다.
		// 상속구조에서는 클래스 변환이 가능하다.
		
		p2.printParent(); //p2라는 레퍼런스로는 Parent에만 접근가능.
		((Child1)p2).printChild1(); // Child 1 내부로 접근하기 위해서는 강제형변환을 해줘야함.
		
		/*
		 * 클래스간에 형변이 가능(단, 상속구조일 경우에만)
		 * 
		 * 1. UpCasting
		 ...
		 *    
		 * 2. DownCasting
		 *    부모타입 -> 자식타입으로 형변환할때는 생략불가능(강제 형변환 해야함)
		 *    ex)	((자식)부모).자식메서드();
		 *    
		 *  다형성을 쓰는 이유
		 *  -부모타입으로부터 파생된 여러가지 타입의 자식객체들의 부모클래스 하나로 다룰 수 있음.
		 */
		
		// * 다형성의 장점
		
		// 다형성 배우기 전
		// Child1객체 2개 Child2객체 2개가 필요함.
		// 객체배열을 이용해보기
		// 2개 맞음
		...
		// 단, 다형성이 적용되면 부모타입 레퍼런스로 모든 자식객체들을 받아줄 수 있다.
		...
		//하나의 부모타입만으로 모든 자식객체들을 다룸으로써, 편리 유지보수하기 좋고 코드길이 감소 == 생산성이 증가
		...
		//바꾸면 ClssCastException에러가 발생함(클래스 형변환이 잘못 되었을 경우 발생하는 에러)
		
		
		
		System.out.println("======반복문========");
		for(int i=0;i<arr.length;i++) {
			//어떤 조건으로 현재 배열 안에 담겨있는 값이 Child1객체인지, Child2객체인지 구분할 수 있나?
...
//			}
			arr[i].print();
			// 단, 오버라이딩을 이용하면 굳이 형변환하지 않아도됨 
			//* 동적바인딩 : 프로그램이 실행되기전에 자동으로 컴파일되면서 정적바인딩(참조변수의 메소드를
			// 				가리킴). 단, 실질적으로 참조하고있는 자식클래스에 해당 메소드가 오버라인딩
