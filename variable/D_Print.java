...
public class D_Print {
	...
	public void printfTest( ) {
		...
		// iNum : xx, iNum2 : xx <<- 요걸 출력할 예정
		// 1. println사용
		System.out.println("iNum : "+iNum1+ ", iNum2 : "+iNum2);
		// 2. printf 사용해서 변경 =>정수값이기때문에 %d로 공간을 확보한다. + 개행이 없으므로 개행문자를 직접 넣어주기
		...
		System.out.printf("dNum : %.10f \n", dNum);
		// %.1f : 소숫점 아래 2번째 줄에서 반올림되어 소숫점 첫번째자리까지만 출력됨 
		// .자릿수로 소숫점을 제어한다 
		
		// 내가 지정한 포맷의 갯수와 종류(자료형), 뒤에오는 변수 혹은 값과 그 종류가 정확히 일치해야한다.
		...
		System.out.printf("%C %S \n", ch, str); //영문자알파벳이 들어가는경우 대문자로 변경해준다.
