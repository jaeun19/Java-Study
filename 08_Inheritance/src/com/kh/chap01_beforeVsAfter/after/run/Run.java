...
public class Run {
	...
	public static void main(String[] args) {
		 ...
		 * - 중복된 코드를 공통적으로 관리하기 때문에 새로운 코드를 추가하거나 수정할때 용이하다.
		 *   =>프로그램 생산성과 유지보수에 크게 기여
		 * - 정의해둔 부모클래스를 통해 좀 더 손쉽게 다른 클래스로의 확장이 가능해짐.
		 * ex)Product 클래스 하나로 백화점에 존재하는 모든 상품객체를 만든다라고 했을 때, 데스크탑, 스마트폰,tv
		 *   3가지 클래스 외에도 냉장고. 에어프라이어, 책, 음식 등 다양한 삼품클래스로 확장시켜줄수가 있다. 
		 *   
		 *   상속의 특징
		 *   -클래스간의 다중상속은 불가능하다(단일상속만 가능)
		 *   -명시되어있지 않지만 모든클래스(자바에서 이미 제공하는 클래스, 내가만든 클래스)는 Object클래스의 후손
