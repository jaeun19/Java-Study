...
public class Book /*extends Object*/ {
	...
	/*
	 * 오버라이딩 
	 * -상속받고있는 부모클래스의 메소드를 자식클래스에서 재정의 하는 것 
	 * -부모가 제공하고 있는 메소드를 자식이 일부 고쳐서 사용하겠다는 의미, 실행시 자식메소드가 우선권을 가짐
	 * 
	 * 오버라이딩 성립 조건
	 * -부모메서드의 메소드명과 동일
	 * -매개변수 자료형, 갯수, 숫서가 동일(매개변수이름 상관없음)
	 * -반환형 동일
	 * -부모메소드의 접근제한자보다 같거나 공유범위가 더 커야한다.
	 * ex)부모메소드의 접근제한자가 protected라면 자식은 public, protected이어야함.
	 * =>최소한 위 조건을 모두 지켜야 오버라이딩이 성립함
	 * 
	 * @Override 어노테이션?
	 * -생략가능(명시를 안해도 부모메서드와 형태가 같으면 오버라이딩 된 것)
	 * -어노테이션을 부이는 이유?(반드시 붙이는게 관례)
	 *  > 잘못 기술했을 경우 오류를 알려주기 때문에 다시한번 검토할 수 있게 해준다.
	 *  > 혹시라도 부모메소드가 나중에 수정되었을 경우 오류로 알려주기 때문에 검토할 수 있게 해준다.
